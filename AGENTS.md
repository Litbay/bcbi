...
# 测试
PYTHONPATH=src .venv/bin/python tests/pubmed/test_pubmed_bulk_export.py
```
...
└── pubmed/               # PubMed 子包 (4 个核心文件)
    ...
    ├── api.py            # 用户 API 实现 (~440 行)
    ├── client.py         # Client + Credentials (~320 行)
    ├── _decode.py        # XML 解码（私有，~540 行）
    ├── _util.py          # 工具函数（私有，~180 行）
    └── py.typed          # 类型提示标记

tests/
└── pubmed/
    ├── test_pubmed_bulk_export.py  # 测试脚本
    └── credentials.py              # 测试凭据（已 gitignore）
...
- `Client` - API 客户端
  ...
  - 类级别 Session 池，复用连接
  - 内置重试和限速逻辑（TokenBucket 令牌桶）

### _decode.py (~540 行，私有)
XML 解码工具：
- `decode_articles()` - 将 XML 解码为文章列表（20 个字段）
- `_decode_article()` / `_decode_book()` - 分别处理 PubmedArticle / PubmedBookArticle
- 一系列 `_get_*` 提取函数（含 books 专属版 `_get_book_*`）
- `_get_text_itertext()` 用于含 `<i>`/`<sup>` 等行内标记的文本提取
- 输出字段请参考 README "文献元数据格式" 一节
...
- 自动分批：webenv 路径 batch_size=1000，pmids 路径限幅 200（避免 URL 过长）
- 限速遵守：TokenBucket 令牌桶，3 req/s 无 key，10 req/s 有 key
